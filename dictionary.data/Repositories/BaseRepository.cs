...
namespace Dictionary.Data.Repositories
{
    public abstract class BaseRepository<TEntity> where TEntity : Base
    {
        ...
        {
            return await _collection.AsQueryable().SingleOrDefaultAsync(predicate);
        }

        public async Task<TEntity> GetByIdAsync(object id)
        {
            return await _collection.AsQueryable().SingleOrDefaultAsync(x => ((string)x.Id).Equals((string)id));
        }
